...
class SQLInsertGenerator(SQLGenerator):
    def __init__(self, operation: Operation, schema_object: SchemaObject) -> None:
        ...
        if self.concurrency_property and operation.store_params.get(self.concurrency_property.name):
            raise ApplicationException(400, f"Versioned properties can not be supplied a store parameters.  schema_object: {schema_object.entity}, property: {self.concurrency_property.name}")
    ...
    def sql(self) -> str:
        ...
        if self.operation.store_params.get(self.concurrency_property.name):
            raise ApplicationException(
                400,
                "When inserting schema objects with a version property the a version must not be supplied as a storage parameter." + 
                f"  schema_object: {self.schema_object.entity}, property: {self.concurrency_property.name}",
            )
        return f"INSERT INTO {self.table_expression}{self.insert_values} RETURNING {self.select_list}"
